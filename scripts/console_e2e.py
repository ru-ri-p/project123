...
Flow: connect -> keygen (browser, Keys & Custody) -> enable customer-key ->
record 2 events (SDK path) -> admin files scoped request -> approve IN THE
BROWSER (tick record, regrant via WebCrypto) -> admin reads exactly the approved
record; out-of-scope stays 403 -> Audit Records verifies the trace.
"""
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        ok("Customer Console" in page.content(), "console page served")

        # 1. Connect
        page.fill("#apikey", api_key)
        page.click("#btn-connect")
        page.wait_for_selector("#orgbadge", state="visible", timeout=5000)
        ok(org_id in page.inner_text("#orgbadge"), "connected; badge shows " + org_id)

        # 2. Keygen in the browser (Keys & Custody screen)
        page.click('nav.screens button[data-k="keys"]')
        ...
        # Admin files a request for record 1 only; cannot read before approval.
        ...
        # 5. Approve in the browser: tick the record, sign as officer, paste key.
        page.click('nav.screens button[data-k="requests"]')
        page.click("#btn-refresh")
        page.wait_for_selector("[data-open]", timeout=5000)
        page.click("[data-open]")
        page.wait_for_selector(".approver", timeout=5000)
        page.click('[data-rec="0"]')
        page.fill(".approver", "officer_e2e")
        page.fill(".keypaste", priv_pem)
        page.evaluate(
            "const t=document.querySelector('#toast');"
            "t.className=''; t.style.display='none'")
        page.click(".approve")
        page.wait_for_selector("#toast.ok >> text=Approved", timeout=15000)
        ok(True, "approved in browser (ticked record, local WebCrypto key release)")

        # 6. The boundary: approved record opens, out-of-scope stays sealed
        r = rq.get(f"{BASE}/v1/admin/access-requests/{req_id}/records/{h1}", headers=ADMIN)
        ok(r.status_code == 200 and r.json()["content"] == {"secret": "value-1"},
        ...
        # 7. Audit Records: verify the customer trace from the console
        page.click('nav.screens button[data-k="trace"]')
        page.fill("#lookup-trace", trace)
        page.click("#btn-verify-trace")
        page.wait_for_selector("#lookup-verdict >> text=CHAIN INTACT", timeout=15000)
